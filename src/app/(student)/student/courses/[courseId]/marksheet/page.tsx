...
const StudentCourseMarksheetPage = ({
    params,
}: {
    params: Promise<{ courseId?: string; courseid?: string; id?: string }>;
}) => {
    const resolved = use(params);
    const courseId = Number(resolved.courseId || resolved.courseid || resolved.id);
    ...
    const overallPct = summary.overall.percentage;
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-6 md:pt-8 space-y-6">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-5 md:gap-6">
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                            ...
                            <p className="text-sm font-semibold text-text-main">
                                {studentName}
                                <span className="text-text-muted font-medium"> · {student.email}</span>
                            </p>
                            <p className="text-[12px] text-text-muted font-medium">
                                {summary.overall.obtainedMarks} / {summary.overall.gradedTotalMarks}{' '}
                                graded · {summary.overall.possibleTotalMarks} total possible
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <SummaryCard
                        icon={ClipboardList}
                        label="Assignments"
                        percentage={summary.assignments.percentage}
                        graded={summary.assignments.graded}
                        total={summary.assignments.total}
                        submitted={summary.assignments.submitted}
                        missing={summary.assignments.missing}
                    />
                    <SummaryCard
                        icon={FileQuestion}
                        label="Quizzes"
                        percentage={summary.quizzes.percentage}
                        graded={summary.quizzes.graded}
                        total={summary.quizzes.total}
                        submitted={summary.quizzes.submitted}
                        missing={summary.quizzes.missing}
                    />
                    <SummaryCard
                        icon={Award}
                        label="Overall"
                        percentage={summary.overall.percentage}
                        graded={summary.overall.gradedItems}
                        total={summary.overall.totalItems}
                        emphasize
                    />
                </div>
                    ...
                    <section className="rounded-2xl border border-border-subtle bg-card-bg overflow-hidden shadow-sm">
                        ...
                        <div className="p-3 sm:p-5">
                            {tab === 'assignments' ? (
                                ...
                                <QuizzesPanel items={data.quizzes} courseId={courseId} />
                            )}
                        </div>
                    </section>
                )}
...
function AssignmentsPanel({
    ...
    if (!items.length) {
        return <EmptyPanel label="No assignments in this course yet" />;
    }
...
function QuizzesPanel({
    ...
    if (!items.length) {
        return <EmptyPanel label="No quizzes in this course yet" />;
    }
...
function EmptyPanel({ label }: { label: string }) {
    return (
        <div className="py-14 text-center text-sm font-medium text-text-muted">{label}</div>
    );
}
...
export default StudentCourseMarksheetPage;
