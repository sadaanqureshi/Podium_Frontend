...
//             <p className="text-text-muted font-black uppercase tracking-[0.2em] text-[10px]">Loading...</p>
//         </div>
//     );
...
export default function EnrolledCoursesPage() {
    ...
    const mappedCourses = useMemo(() => {
        return enrolledCourses.map((item: any) => {
            ...
            const overallTotal = item.progress?.overall?.total || 0;
            const overallCompleted = item.progress?.overall?.completed || 0;
            const calculatedProgress = overallTotal > 0 ? Math.round((overallCompleted / overallTotal) * 100) : 0;
            ...
            return {
                ...
                avgRating: item.course.avgRating || 0,
                rating: item.course.avgRating || 0,
                progress: calculatedProgress, // Dynamic progress here
                totalLectures: item.course.totalLectures || 0,
            };
        }) as any[];
    }, [enrolledCourses]);
