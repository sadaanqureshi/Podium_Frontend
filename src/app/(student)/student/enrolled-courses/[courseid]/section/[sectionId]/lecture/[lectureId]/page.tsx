...
const LectureDetailPage = ({ params }: { params: Promise<any> }) => {
    ...
    if (!lecture && !isCourseLoading) return (
        <div className="h-screen flex flex-col items-center justify-center p-6 text-center bg-app-bg transition-colors duration-300">
            ...
            <Link href={`/teacher/assigned-courses/${courseId}`} className="px-8 py-3 bg-accent-blue text-white rounded-2xl font-black text-xs uppercase shadow-xl shadow-accent-blue/20 hover:bg-hover-blue transition-all">Back to Dashboard</Link>
        </div>
    );
