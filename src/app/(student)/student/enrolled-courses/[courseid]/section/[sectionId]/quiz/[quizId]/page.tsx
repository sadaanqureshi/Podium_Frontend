...
const StudentQuizDetailPage = ({ params }: { params: Promise<any> }) => {
    ...
    const handleQuizSubmit = async (answers: any[]) => {
        if (submitLock.current || submitting || isSubmitted) return;
    ...
    const canAttempt = windowState === 'open' || windowState === 'open-always';
                    ...
                    <div className="bg-card-bg border border-border-subtle rounded-2xl p-8 md:p-12 text-center space-y-6 shadow-sm">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-text-muted text-sm font-medium max-w-md mx-auto leading-relaxed">
                                {resultData
                                    ? `You scored ${resultData.totalMarksObtained ?? resultData.total_marks_obtained ?? '—'} points. Review details below.`
                                    : 'You finished this quiz. Your teacher will grade it soon — tap Check Status to refresh.'}
                            </p>
                        </div>
