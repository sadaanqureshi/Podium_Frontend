...
const AdminResourceDetailPage = ({ params }: { params: Promise<any> }) => {
    ...
    // 👉 THE FIX FOR ENDLESS LOADING
    const isPageLoading = (!fullData && reduxLoading?.courseContent?.[courseIdNum] !== false);
    ...
    if (!resource && !isPageLoading) return (
        <div className="h-full min-h-[80vh] flex flex-col items-center justify-center p-6 text-center bg-app-bg">
            ...
            <p className="text-text-muted text-sm mb-6 max-w-md">Note: Resource data not found in system.</p>
            <Link href={`/admin/courses/${id}`} className="text-accent-blue font-bold text-sm hover:underline transition-all">Return to Course</Link>
        </div>
    );

    return (
        <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-top-4 pb-20 bg-app-bg min-h-screen text-text-main">
